...
export const RosterChainView: React.FC<RosterChainViewProps> = ({ complaint }) => {
  const steps = [
    ...
    },
    {
      ...
      value: complaint.busNumber || 'KL-15-A-4021',
      subtext: complaint.incidentTime || 'Incident Time',
      icon: <Bus className="w-4 h-4 text-amber-600" />,
    },
    {
      title: 'Route Master',
      value: `${complaint.routeFrom} ➔ ${complaint.routeTo}`,
      subtext: `Code: ${complaint.routeCode || 'RT-102'}`,
      icon: <MapPin className="w-4 h-4 text-blue-600" />,
    },
    {
      title: 'Assigned Depot',
      value: complaint.depotName || 'Trivandrum Central Depot',
      subtext: `ID: ${complaint.depotId || 'DEP-TVM'}`,
      icon: <Building2 className="w-4 h-4 text-purple-600" />,
    },
    {
      title: 'Duty / Shift Roster',
      value: `Duty ID: ${complaint.dutyId || 'D-104'}`,
      subtext: 'Morning Shift (06:30 - 14:30)',
      icon: <Calendar className="w-4 h-4 text-indigo-600" />,
    },
    {
      title: 'Conductor PEN (Employee)',
      value: complaint.conductorPen || 'PEN-88421',
      subtext: 'Role-Protected Record (Rajesh Kumar)',
      icon: <UserCheck className="w-4 h-4 text-emerald-600" />,
      protected: true,
    },
    {
      title: 'Current Case Owner',
      value: complaint.assignedOwner || 'Depot Manager TVM',
